...
class WPF_WCS_Gifting extends WPF_Integrations_Base {
	...
	public function user_register( $user_meta, $user_id ) {
		...
		if ( $user_meta['user_email'] !== $user->user_email ) {

			$user_meta = wp_fusion()->user->get_user_meta( $user_id );

			wpf_log( 'info', $user_id, 'Creating new contact record for gift rcipient with email <strong>' . $user_meta['user_email'] . '</strong>' );

			$user_meta['first_name'] = null; // force these blank to override the auto-name detection in WPF_User::maybe_set_first_last_name.
			$user_meta['last_name']  = null;

			$user_meta['billing_first_name'] = null; // Prevent these from overwriting the recipient details.
			$user_meta['billing_last_name']  = null;
			$user_meta['billing_email']      = null;
