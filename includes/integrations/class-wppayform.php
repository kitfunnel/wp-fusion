...
class WPF_WPPayForm extends IntegrationManager {
	...
	public function __construct() {
		...
		$this->description = sprintf( __( 'WP Fusion syncs your WPPayForms submissions with %s and applies tags based on payments and subscription statuses.', 'wp-fusion' ), wp_fusion()->crm->name );
	...
	public function getSettingsFields( $settings, $form_id = null ) {
		...
		$form_fields = \WPPayForm\App\Models\Form::getEditorShortCodes( $form_id );
		$user_fields = $form_fields[0]['shortcodes'];
		$form_fields = array_merge( $user_fields, $form_fields[1]['shortcodes'] );
...
wp_fusion()->integrations->{'wppayform'} = new WPF_WPPayForm();
