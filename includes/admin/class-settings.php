...
class WPF_Settings {
 ...
 $license_data->license = 'valid';
	...
	public function get_settings( $settings, $options ) {
		...
		*/
		...
		$settings['restrict_content'] = array(
			...
			'tooltip' => sprintf( __( 'WP Fusion includes many features for restricting access to content based on tags in %s. However if you are not using these features (you only want to sync data with %s), you can un-check this setting to disable the access control interfaces and features.', 'wp-fusion' ), wp_fusion()->crm->name, wp_fusion()->crm->name ),
		);
		...
		$settings['default_not_logged_in_redirect'] = array(
			...
			'desc'    => __( 'A redirect URL specified here will take priority in cases where a visitor was denied access to a piece of content due to not being logged in.', 'wp-fusion' ),
			'type'    => 'text',
			'section' => 'main',
		);
		...
		$settings['restricted_message'] = array(
			...
			'desc'          => __( '<a href="https://wpfusion.com/documentation/getting-started/access-control/#restricted-content-message" target="_blank">Restricted content message</a> for when a redirect isn\'t set. Use <code>[the_excerpt]</code> or <code>[the_excerpt length="120"]</code> to display an excerpt of the restricted content.', 'wp-fusion' ),
		...
		*/
		...
		if ( in_array( 'leads', wp_fusion()->crm->supports ) ) {
			...
			$settings['leads'] = array(
				...
				'desc'    => sprintf( __( 'Sync guest form submissions to Leads in %s.', 'wp-fusion' ), wp_fusion()->crm->name ),
				'type'    => 'checkbox',
				'section' => 'integrations',
				'tooltip' => sprintf( __( 'WP Fusion now supports creating Leads in %s (instead of Contacts) when a lead form is submitted by a guest. When this setting is enabled, all form submissions will be synced to a Lead record, and any tags will be applied to the lead.', 'wp-fusion' ), wp_fusion()->crm->name ),
			);
		...
		*/
		...
		$settings['js_leadsource_tracking'] = array(
			...
			'desc'    => __( 'Enqueue a script to handle setting the <a href="https://wpfusion.com/documentation/tutorials/lead-source-tracking/" target="_blank">lead source tracking cookies</a>. This fixes tracking on WP Engine, Flywheel, and other hosts that sanitize UTM parameters out of URLs.', 'wp-fusion' ),
			'type'    => 'checkbox',
			'section' => 'advanced',
		);
		...
		if ( is_multisite() ) {
			...
			$settings['multisite_prefix_keys'] = array(
				...
				'desc'    => __( 'Prefix all WP Fusion usermeta keys with the blog ID of the current site in the network.', 'wp-fusion' ),
	...
	public function show_field_oauth_authorize( $id, $field ) {
		...
		echo '<a id="' . esc_attr( $field['slug'] ) . '-auth-btn" class="button ' . ( isset( $field['dis'] ) ? 'button-disabled' : 'button-primary' ) . '" href="' . esc_url( $field['url'] ) . '">' . sprintf( esc_html__( 'Authorize with %s', 'wp-fusion' ), $field['name'] ) . '</a><br />';
		echo '<span class="description">' . sprintf( esc_html__( 'You\'ll be taken to %s to authorize WP Fusion and generate access keys for this site.', 'wp-fusion' ), $field['name'] ) . '</td>';
	...
	public function show_field_edd_license( $id, $field ) {
		...
		if ( 'valid' === $this->license_status ) {
			echo '<a id="edd-license" data-action="edd_deactivate" class="button">' . esc_html__( 'Deactivate License', 'wp-fusion' ) . '</a>';
		} else {
