...
class WPF_Admin_Interfaces {
	...
	public function taxonomy_form_fields( $term ) {
		...
		<table class="wpf-settings-table form-table wpf-meta">
			...
			<tbody>
				...
				<tr class="form-field">
					<th scope="row" valign="top"><label for="lock_content"><?php esc_html_e( 'Restrict access to archives', 'wp-fusion' ); ?></label></th>
					<td>
				...
				<tr class="form-field">
					<th scope="row" valign="top"><label for="lock_posts"><?php esc_html_e( 'Restrict access to all posts', 'wp-fusion' ); ?></label></th>
					<td>
	...
	public function restrict_content_checkbox( $post, $settings ) {
		...
		$message = sprintf( __( 'Users must be logged in to view this %s', 'wp-fusion' ), $post_type_object_label );
	...
	public function required_tags_select( $post, $settings ) {
		...
		echo '<p class="wpf-required-tags-select"><label' . ( $settings['lock_content'] != true ? '' : ' class="disabled"' ) . ' for="wpf-allow-tags"><small>' . esc_html__( 'Required tags (any)', 'wp-fusion' ) . ':</small>';
		...
		echo '<p class="wpf-required-tags-select"><label' . ( $settings['lock_content'] != true ? '' : ' class="disabled"' ) . ' for="wpf-allow-tags-all"><small>' . esc_html__( 'Required tags (all)', 'wp-fusion' ) . ':</small>';
	...
	public function page_redirect_select( $post, $settings, $disabled = false ) {
		...
		echo '<p class="wpf-page-redirect-select"><label for="wpf-redirect"><small>' . esc_html__( 'Redirect if access is denied:', 'wp-fusion' ) . '</small>';

		echo '<span class="dashicons dashicons-editor-help wpf-tip wpf-tip-bottom" data-tip="' . esc_attr__( 'If you do not specify a redirect WP Fusion will try to replace the content area of the post with the restricted content message configured in the WP Fusion settings.', 'wp-fusion' ) . '"></span></label>';
	...
	public function external_redirect_input( $post, $settings ) {
		...
		echo '<p class="wpf-external-redirect-input"><label for="wpf-redirect-url"><small>' . esc_html__( 'Or enter a URL below:', 'wp-fusion' ) . '</small></label>';
	...
	public function apply_tags_select( $post, $settings ) {
		...
		echo '<p class="wpf-apply-tags-select"><label for="wpf-apply-tags"><small>' . sprintf( __( 'Apply tags when a user views this %s', 'wp-fusion' ), esc_html( $post_type_object_label ) ) . ':</small></label>';
		...
		echo '<p class="wpf-apply-tags-select"><label for="wpf-remove-tags"><small>' . sprintf( __( 'Remove tags when a user views this %s', 'wp-fusion' ), esc_html( $post_type_object_label ) ) . ':</small></label>';
