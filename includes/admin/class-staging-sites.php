...
class WPF_Staging_Sites {
	...
	public function __construct() {
		...
		add_filter( 'wpf_get_setting_site_url', array( $this, 'get_site_url' ) );
	 ...
	 */
	public function get_site_url( $value ) {

		return str_replace( '_[wpf_siteurl]_', '', $value );
	...
	public static function get_duplicate_site_lock_key() {
		...
		return $scheme . substr_replace( $site_url, '_[wpf_siteurl]_', strlen( $site_url ) / 2, 0 );
	}
	...
	public function show_staging_notice() {
		...
		if ( $this->is_duplicate_site() && current_user_can( 'manage_options' ) && ! wpf_get_option( 'dismissed_wpf-staging-notice' ) ) {
			...
			esc_html_e( 'Disable staging mode', 'wp-fusion' );
